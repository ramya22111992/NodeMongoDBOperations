...
const { getUser, createUser, getAllUsers,deleteUser,deleteAllUsers,saveUserDocument} = require('../services/userService')
const {removeAllToDosOfUser,removeSingleToDo,getToDo,saveToDo,createToDo}=require('../services/todoService');
    ...
    if (userToDos.n != 0) {
      ...
      getUser(req.params.userId).then(userInfo => {
        if (userInfo) {
          ...
          saveUserDocument(userInfo).then(savedUser=>{
            res.setHeader('Content-Type', 'application/json');
    ...
    if (deletedToDo) {
      getUser(req.params.userId).then(user => {
        if (user) {
          ...
          saveUserDocument(user).then(savedUser=>{
            res.setHeader('Content-Type', 'application/json');
...
exports.updateToDo = (req, res, next) => {
  getToDo(req.params.todoId).then(todo => {
    if (todo) {
      ...
      saveToDo(todo).then(updatedToDo=>{
...
exports.getPost = (req, res, next) => {
  getUser(req.params.userId).populate('posts').then(user => {
    if (user) {
      res.setHeader('Content-Type', 'application/json');
      res.status(200).json({
